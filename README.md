...
Add an element to a value_set:
```shell
psabpf-ctl value_set insert pipe ID VALUE_SET_NAME value DATA
```
Delete an element from a value_set:
```shell
psabpf-ctl value_set delete pipe ID VALUE_SET_NAME value DATA
```
List all elements in the value_set:
```shell
psabpf-ctl value_set get pipe ID VALUE_SET_NAME
```
- ID - ID of the pipeline, natural number.
- VALUE_SET_NAME - name of the value_set.
- DATA - the value_set value. Values can be specified in binary, octal, decimal or hexadecimal format.