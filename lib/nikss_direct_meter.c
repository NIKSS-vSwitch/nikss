...
void nikss_direct_meter_ctx_init(nikss_direct_meter_context_t *dm_ctx)
{
    ...
    memset(dm_ctx, 0, sizeof(nikss_direct_meter_context_t));
                                       ...
                                       nikss_meter_entry_t *dm)
{
    ...
    if (entry->direct_meters == NULL) {
        tmp_ptr = malloc(sizeof(nikss_direct_meter_entry_t));
    } else {
        tmp_ptr = realloc(entry->direct_meters, (entry->n_direct_meters + 1) * sizeof(nikss_direct_meter_entry_t));
    }
    if (tmp_ptr == NULL) {
    ...
    nikss_meter_entry_init(&entry->direct_meters[idx].meter);
...
nikss_direct_meter_context_t *nikss_direct_meter_get_next_ctx(nikss_table_entry_ctx_t *ctx, nikss_table_entry_t *entry)
{
    ...
    memcpy(&entry->current_direct_meter_ctx,
           ...
           sizeof(nikss_direct_meter_context_t));
