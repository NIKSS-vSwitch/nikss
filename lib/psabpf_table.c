...
static int count_direct_objects(nikss_table_entry_ctx_t *ctx)
{
    ...
    const struct btf_type *value_type = NULL;
                                  ...
                                  nikss_table_entry_t *entry, const struct btf_type *value_type)
{
    ...
    for (int i = 0; i < entries; i++, member++) {
        ...
        if (entry_ref_used) {
            entry_ref_used = false;  /* NOLINT(clang-analyzer-deadcode.DeadStores) */
...
static int ternary_table_add_tuple_and_open(nikss_table_entry_ctx_t *ctx, const uint32_t tuple_id)
{
    ...
    struct bpf_create_map_attr attr = {
            ...
            .btf_key_type_id = ctx->table.map_key_type_id,
            .btf_value_type_id = ctx->table.map_value_type_id,
    };
